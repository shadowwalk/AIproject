...
double runtimeUCSO,runtimeUCSET,runtimeGSO,runtimeGSET,runtimeASO,runtimeASET;
...
clock2 = clock();
runtimeUCSO=(double)(clock2 - clock1)*(1/(double)CLOCKS_PER_SEC) ;


clock1 = clock();
memCostUCSET = UCSBYET(&myEngine[0],&myTask[0],&myMapNode[0]);
clock2 = clock();
runtimeUCSET=(double)(clock2 - clock1)*(1/(double)CLOCKS_PER_SEC) ;


clock1 = clock();
memCostGSO = GSBYO(&myEngine[0],&myTask[0],&myMapNode[0]);
clock2 = clock();
runtimeGSO=(double)(clock2 - clock1)*(1/(double)CLOCKS_PER_SEC) ;

clock1 = clock();
memCostGSET = GSBYET(&myEngine[0],&myTask[0],&myMapNode[0]);
clock2 = clock();
runtimeGSET=(double)(clock2 - clock1)*(1/(double)CLOCKS_PER_SEC) ;

clock1 = clock();
memCostASO = ASBYO(&myEngine[0],&myTask[0],&myMapNode[0]);
clock2 = clock();
runtimeASO=(double)(clock2 - clock1)*(1/(double)CLOCKS_PER_SEC) ;

clock1 = clock();
memCostASET = ASBYET(&myEngine[0],&myTask[0],&myMapNode[0]);
clock2 = clock();
runtimeASET=(double)(clock2 - clock1)*(1/(double)CLOCKS_PER_SEC) ;
...
int UCSBYO(const engine *engineInfoP, const task *taskInfoP,mapNode *myMapNodeP)
{
	...
	while( finish == FALSE){
		...
		for (int i=0;i<nextEmptyIndex;i++){
			...
			else if(myMapNodeP[i].explored == TURE && myMapNodeP[i].nodeNo>largestBranchNode){//not leaf node
				largestBranchNode = myMapNodeP[i].nodeNo;
			}
		}
		NodeSearch(engineInfoP,taskInfoP,myMapNodeP,lowestNodeNo,&nextEmptyIndex);
		
		if(myMapNodeP[lowestNodeNo].goal == TURE){
			finish = TURE;
...
int GSBYO(const engine *engineInfoP, const task *taskInfoP,mapNode *myMapNodeP)
{
	...
	while( finish == FALSE){
		...
		for (int i=0;i<nextEmptyIndex;i++){
			if(myMapNodeP[i].goal == TURE && finish == FALSE){
				finish = TURE;
				goalNodeNo = i;
		...
		/*
		for(int i=0;i<engineNum;i++){
			if(myMapNodeP[myMapNodeP[lowestNodeNo].childNo[i]].goal == TURE){
				finish = TURE;
		...
		for (int i=0;i<engineNum;i++){
			if(myAvailTask[currentTaskIndex].availEngineNo[i] == 1){
				...
				if(currentTaskIndex == taskNum-1){
					searchNodeP[*nextEmpty].goal= TURE;
				}
